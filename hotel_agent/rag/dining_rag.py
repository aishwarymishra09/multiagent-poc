...
class DiningRAG:
    ...
    async def query(self, query: str, k: int = settings.MAX_RESULTS) -> List[Dict[str, Any]]:
        """Query the vector store for relevant information"""
        try:
            # Get embedding for the query
            query_embedding = await similarity_calculator.get_embedding(query)
            # print("______ query_embedding:{}".format(query_embedding))
        ...
        for attempt in range(settings.MAX_RETRIES):
            try:
                ...
                for doc in documents:
                    similarity = await similarity_calculator.find_most_similar(
                        query,
